...
:imagesdir: ../images
:sectnums:
:description: Main page for the Dialogue Branch Language Definition.

At its core, Dialogue Branch is similar to a programming or "scripting" language. The Dialogue Branch Language is defined by a set of rules that dictates what a validly specified Dialogue Branch Dialogue is. It tells you how to specify _Statements_ that can be uttered by _Agents_, and how to define various _Reply_ options.

The numbered sections below describe all of the different concepts in a Dialogue Branch script, starting from the basics, and dealing progressively with more advanced topics.

== Basics & Terms
A Dialogue Branch script is essentially a definition of a series of dialogue steps (that we refer to as _Node_s) linked together through user replies.
...
* +title+ -- a String matching `[A-Za-z0-9_-]+` that uniquely identifies this _Node_ within this dialogue (matching is case-insensitive).
* +speaker+ -- a String that defines the name of the _Agent_ speaking in this _Node_. Required for every node, *except* a node titled xref:_ending_a_dialogue[`End`], whose speaker is ignored even if present.

You are free to define other key-value pairs that might serve as meta-data in your application. The visual editor in the web client (see xref:dlb-web:index.adoc[Dialogue Branch Web Services]) additionally recognises the following reserved tags:
...
NOTE: The reserved tag is spelled +colorId+ (lowercase `d`). Some older example content in this repository still uses the legacy spelling +colorID+ -- the parser accepts either (any header key you write is stored as-is), but only +colorId+ is recognised by the visual editor.
...
A `.dlb` file does not need to be registered anywhere -- any file with a `.dlb` extension found anywhere inside a language folder of a xref:dlb-project.adoc[Dialogue Branch Project] is automatically picked up and parsed.
...
* There must be one *Node* with the title "__Start__" (matched case-insensitively, so `start`, `Start` or `START` all satisfy this requirement).
...
=== Starting a Dialogue
Every Dialogue Branch script must include a Node with title "__Start__". Applications that execute Dialogue Branch scripts can choose this as the default starting node for a conversation (or ignore it, and start somewhere else, it's your party). For example:
 ...
 * The user chooses a *Reply* option that leads to a *Node* with title "__End__" (matched case-insensitively) (user has the last say).
...
Be careful with using the shorthand form `\<<if $variableName>>`, because it is __not strict__, while for example `\<<if $variableName == false>>` is strict. This means that if the variable `$variableName` has not been assigned a value, the following will happen:
...
At runtime, exactly one of the clauses is chosen at random (proportional to its weight) and only that clause's content is executed -- the others are skipped entirely. A `\<<random>>` block may appear anywhere a `\<<set>>` or `\<<if>>` statement may appear, and each clause may itself contain nested `\<<if>>`, `\<<set>>` or further `\<<random>>` statements. It is not valid inside the command section of a *Reply* (see <<Replies>> below).
...
NOTE: The optional third section of a *Reply* (after the second `|`) may only contain `\<<set>>` and/or `\<<action>>` statements -- `\<<if>>` and `\<<random>>` are not valid there. If you need conditional logic around a reply, express it in the *Node*'s body instead (e.g. by using `\<<if>>` to decide which replies to show).
...
In this example, the first *Reply* option would take the user to the *Node* labeled "__Start__" of the Dialogue labeled "__CarsDialogue__". So, in this case, there should be a file named "__CarsDialogue.dlb__" in the same folder as the current .dlb script.
...
Note that you don't __have to__ link to the "Start" *Node* of a dialogue script, and you can choose any valid *Node* name.
