...
== Introduction
The "Core" of the Dialogue Branch software is a library that can read and execute Dialogue Branch (.dlb) scripts. This core library is written in Java, and its source lives in this monorepo under https://github.com/dialoguebranch/platform/tree/main/packages/core[`packages/core`]. It is published to Maven Central as `com.dialoguebranch:dlb-core-java`.
...
For the exact syntax of the `.dlb` script format this library parses and executes, see the xref:dlb-language:index.adoc[Dialogue Branch Language Definition].

== Documentation
Javadoc for the library can be generated locally with `./gradlew javadoc` (output in `packages/core/build/reports/javadoc/`). Since the library is published to Maven Central, its Javadoc is also browsable via https://javadoc.io/doc/com.dialoguebranch/dlb-core-java[javadoc.io] for any released version, without a local build.
