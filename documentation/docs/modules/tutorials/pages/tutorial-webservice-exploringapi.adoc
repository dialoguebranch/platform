...
NOTE: Which end-points you're allowed to call, and whether you can specify a `delegateUser` parameter to act on behalf of another user, depends on which of the `client`/`editor`/`admin` roles are assigned to your test user -- see xref:dlb-web:authentication.adoc[Authentication].
