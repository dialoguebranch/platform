...
This tutorial assumes that you've successfully installed the Web Service and created a test user, e.g. by following the xref:tutorial-webservice-installation.adoc[Tutorial: Dialogue Branch Web Service - Installation].
...
=== Authenticating
All API end-points (other than `/info/all` and a few infrastructure end-points) require a valid https://jwt.io/[JSON Web Token] (JWT), issued by Keycloak, in the `Authorization: Bearer <token>` header of every call -- see xref:dlb-web:authentication.adoc[Authentication] for the full picture. The bundled Swagger UI is itself registered as a Keycloak client, so you can authenticate without writing any client code:
...
NOTE: Which end-points you're allowed to call, and whether you can specify a `delegateUser` parameter to act on behalf of another user, depends on which of the `participant`/`editor`/`admin` roles are assigned to your test user -- see xref:dlb-web:authentication.adoc[Authentication].
...
* Unfold the */dialogue/start* endpoint and click on "_Try it out_". There are six parameters that can be entered at this point:
...
* When you've filled in "*default-test*" as `projectSlug`, "*basic*" as `dialogueName`, "*en*" as `language` and any valid `timeZone`, you can click "_Execute_", resulting in a JSON Response similar to the following:
...
* Next, we want to advance the dialogue by sending the reply "_Nice to meet you Martin McOwl._". For this we will unfold the */dialogue/progress* end-point, and click on "_Try it out_". Besides the optional `delegateUser` parameter explained earlier, there are three parameters for this call:
...
* When you've filled in everything you can click "_Execute_", resulting in a JSON Response similar to the following:
...
* Fill in the `loggedDialogueId` and the `loggedInteractionIndex` from the response you got in the previous step, and press "_Execute_".
