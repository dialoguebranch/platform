...
. On the *Role mapping* tab, click *Assign role*, switch the filter to *Filter by clients*, and assign one or more of the `dlb-web-service` client roles: `client`, `editor`, `admin` (see xref:dlb-web:authentication.adoc[Authentication] for what each role permits -- for exploring the full API, assign `admin`).
