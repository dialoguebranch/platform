...
Cut the actual GitHub release with `infrastructure/release/release-github.sh`, in two steps, because `main`'s branch ruleset requires the `all-green` status check on every commit and so rejects a direct push, including a release commit. Step one (the default, no flags): prompts for a major/minor/patch release type, bumps `global.json` accordingly, splits `CHANGELOG.md`'s `[Unreleased]` section into a dated version section, commits both on a new `release/vX.Y.Z` branch, pushes it, and opens a PR against `main` (title `Release vX.Y.Z`, body the new changelog section). Step two, `--finish`, run after that PR has merged and CI is green: reads the already-bumped version back out of `global.json`, tags `vX.Y.Z`, re-points the floating `latest` tag, pushes both, and creates the GitHub release (via `gh`) using that changelog section as the release notes. Both steps must be run from a clean `main` in sync with `origin/main`, and each asks for confirmation before pushing anything.

The same two steps also exist as GitHub Actions, mirroring the script exactly rather than redesigning it ([#265](https://github.com/dialoguebranch/platform/issues/265)): **`release-prepare.yml`** (`workflow_dispatch`, pick a `major`/`minor`/`patch` bump) does step one; **`release-finish.yml`** (`pull_request: closed`, filtered to a merged `release/v*` head branch) does step two automatically once that PR merges, closing the "remember to come back and finish it" gap the manual `--finish` step has. Both need a repository secret, `RELEASE_PAT` (a personal access token, not the default `GITHUB_TOKEN`) — pushes and PRs/releases created with the default token don't trigger other workflows, so the release PR's required `all-green` check would never run, and a future npm-publish-on-`release: published` workflow (tracked separately as [#230](https://github.com/dialoguebranch/platform/issues/230)) would never fire either. The local script remains as a fallback/manual escape hatch alongside the workflows.
